...
\begin{homeworkProblem}[Basic IPython usage]
\begin{enumerate}
\item Try the 2 iPython modes
\item Create new cells above and below your current one
\item Try the following:
  \begin{enumerate}
  \item In the 1st cell, set the variable \hilight{name} to your name. Execute the cell.
  \item In the 2nd cell, execute \hilight{print name}. Execute the cell.
  \item In the 3rd cell, redefine \hilight{name} to something else. Execute the cell.
  \item What will happen if you now rerun 2nd cell? Why? Would this be different if the notebook was a regular Python script? 
  \end{enumerate}
\end{enumerate}
\end{homeworkProblem}

\begin{homeworkProblem}[Basic DataFrames]
\begin{enumerate}
\item Load the quasar dataset into a pandas DataFrame
\item Use \hilight{head}, \hilight{tail} and \hilight{describe}
\item Try out what happens if you load the DataFrame without using the correct \hilight{sep} argument
\item Use the loc method to... 
  \begin{enumerate}
  \item select rows 5-10.
  \item select all rows of the sample column.
  \item select rows 5-10 of chrom and pos.
  \end{enumerate}
\item Use the iloc method to select the first 5 rows and columns 
\item Save a subset of df into df2 and compare the outputs of describe. 
\end{enumerate}
\end{homeworkProblem}

\begin{homeworkProblem}[Counting and Boolean Indexing]
\begin{enumerate}
\item How many different genes are there in the dataset?
\item Use boolean indexing to select all rows with FAO $>$ 50.
\item How many rows are there with FAO $>$ 500? And for FAO $>$ 1500? 
\item How many rows have an effect of either 'STOP\_GAINED' or 'FRAME\_SHIFT'? Hint: this is possible in one command by using the \hilight{.isin(list)} method.
\end{enumerate}
\end{homeworkProblem}

\begin{homeworkProblem}[Column creation]
\begin{enumerate}
\item Replace the "Effect\_Impact" column with a lowercase version of itself. Hint: the \hilight{.str} method exposes a column as a string so you can use all the standard Python string functions.
\item Create a new column called DP as the sum of FAO and FDP.
\item Create a new column called len as the combined length of ref and alt.
\item Write a function that parses the Effect from a row and returns "strong" if it's in 'STOP\_GAINED' or 'FRAME\_SHIFT', otherwise "weak". Test the function using a dictionary (to simulate a row).
\item Create a new column "Binary\_Impact" by applying the function using \hilight{df.apply(function, axis=1)}.
\end{enumerate}
\end{homeworkProblem}
